...
public class TestProxy extends WebappTestCase {
    ...
    /**
     * Verifies that remote includes are processed correctly
     */
    public void testRemoteInclude() throws IOException {
        final URL url = new URL(getServerUrl() + "/proxy?fmt=svg&idx=0&src=" + getServerUrl() + "/resource/testInclude.puml");
        final URLConnection conn = url.openConnection();
        // Analyze response
        // Verifies the Content-Type header
        // Get the content and verify its size
        String diagram = getContentText(conn);
        try (PrintWriter out = new PrintWriter("testInclude.svg")) {
